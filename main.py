...
# База данных шрифтов (только проверенные, без багов)
FONTS_DATA = {
...
<head>
    ...
    <style>
        ...
        /* --- AdSense Layout --- */
        .ad-banner {{ width: 100%; max-width: 970px; height: 90px; background: rgba(255,255,255,0.01); border: 1px dashed rgba(255,255,255,0.05); margin-bottom: 30px; display: flex; align-items: center; justify-content: center; color: #333; font-size: 10px; text-transform: uppercase; letter-spacing: 2px; }}
        ...
        .ad-left {{ left: 20px; }}
        .ad-right {{ right: 20px; }}
        .ad-bottom {{ width: 100%; max-width: 970px; height: 250px; margin-top: 50px; background: rgba(255,255,255,0.01); border: 1px dashed rgba(255,255,255,0.05); display: flex; align-items: center; justify-content: center; color: #333; font-size: 10px; text-transform: uppercase; letter-spacing: 2px; }}
        ...
        .card {{ 
            ...
            transition: 0.3s cubic-bezier(0.2, 0.8, 0.2, 1); cursor: pointer; animation: slideIn 0.5s ease;
        }
        ...
        .card span {{ font-size: 1.6rem; color: var(--text); font-weight: 500; }}
        .copy-btn {{ 
...
<body>
    <div class="ad-banner">Top Leaderboard Ad Space</div>
    
    <div class="ad-side ad-left">Skyscraper Ad Space (Left)</div>
    
    <div class="ad-side ad-right">Skyscraper Ad Space (Right)</div>

    <div class="container">
        <h1>FONT FLOW</h1>
        <div class="tagline">Copy. Paste. Shine.</div>
        ...
        <div class="ad-bottom">Large Rectangle Ad Space (Bottom)</div>
    </div>

    <script>
        const fontKeys = {list(FONTS_DATA.keys())};
        const fontMaps = {FONTS_DATA};
        ...
        function copyText(text, card) {{
            ...
            btn.style.color = "#fff";
            btn.style.background = "#00ff88"; // Neon green back
            setTimeout(() => {{ 
                btn.innerText = "Copy"; 
                btn.style.color = "var(--primary)"; 
                btn.style.background = "rgba(0,255,136,0.1)"; // Transparent neon green
            }}, 1500);
        }}
    </script>
</body>
